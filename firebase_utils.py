    ...
    if not firebase_admin._apps:
        ...
        if cred_json:
            try:
                ...
                FIREBASE_FULL = True
                print("✅ Firebase FULL mode — initialized from environment variable JSON")
            except Exception as e:
                print(f"⚠️  Firebase initialization from environment variable failed: {e}")

        if not FIREBASE_FULL:
            if os.path.exists(_CRED_PATH):
                ...
                print(f"⚠️  No serviceAccountKey.json found at '{_CRED_PATH}' and no FIREBASE_CRED_JSON env var")
