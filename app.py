...
class MockCamera:
    ...
    def read(self):
        ...
        # Drawing UI decoration on simulated stream
        cv2.putText(frame, "FirePro AI - Simulated Feed", (30, 45), 
                    cv2.FONT_HERSHEY_DUPLEX, 0.7, (255, 107, 53), 2)
        
        # Grid pattern
        for y in range(80, self.height, 80):
            cv2.line(frame, (0, y), (self.width, y), (30, 30, 45), 1)
        for x in range(80, self.width, 80):
            cv2.line(frame, (x, 0), (x, self.height), (30, 30, 45), 1)

        # Dynamic simulation depending on time
        cycle = (time.time() - self.start_time) % 24.0
        
        # Draw dynamic items
        if cycle < 8.0:
            # Mode A: Person walking (green circle)
            cx = int(self.width / 2 + 180 * np.sin(time.time() * 1.5))
            cy = int(self.height / 2 + 100 * np.cos(time.time() * 1.5))
            cv2.circle(frame, (cx, cy), 35, (0, 200, 100), -1)
            cv2.putText(frame, "Simulated Person", (cx - 70, cy - 45), 
                        cv2.FONT_HERSHEY_DUPLEX, 0.55, (0, 200, 100), 1)
            cv2.putText(frame, "Status: Monitoring - No Alerts", (30, self.height - 30), 
                        cv2.FONT_HERSHEY_DUPLEX, 0.55, (0, 255, 0), 1)
        elif cycle < 16.0:
            # Mode B: Fire simulation (orange pulsing polygon)
            pulse = int(15 * np.sin(time.time() * 12))
            pts = np.array([
                [320, 160 + pulse],
                [380 + pulse, 280],
                [260 - pulse, 280]
            ...
            cv2.putText(frame, "Simulated Fire Alert", (240, 135), 
                        cv2.FONT_HERSHEY_DUPLEX, 0.55, (0, 60, 255), 1)
            cv2.putText(frame, "Status: WARNING - FIRE DETECTED", (30, self.height - 30), 
                        cv2.FONT_HERSHEY_DUPLEX, 0.55, (0, 0, 255), 1)
        else:
            # Mode C: Smoke simulation (gray circles rising)
            t_offset = time.time() * 4
            for i in range(3):
                cx = int(320 + 20 * np.sin(t_offset + i))
                cy = int(240 - 40 * i - 10 * (t_offset % 4))
                r = int(30 + 10 * i + (t_offset % 4) * 2)
                cv2.circle(frame, (cx, cy), r, (128, 128, 128), -1)
            cv2.putText(frame, "Simulated Smoke Alert", (230, 80), 
                        cv2.FONT_HERSHEY_DUPLEX, 0.55, (180, 180, 180), 1)
            cv2.putText(frame, "Status: WARNING - SMOKE DETECTED", (30, self.height - 30), 
                        cv2.FONT_HERSHEY_DUPLEX, 0.55, (0, 100, 255), 1)
...
def _detection_loop():
    ...
    if camera_source != "mock":
        try:
            ...
            print(f"[Camera] Failed to open camera source {camera_source}: {e}")

    if cap is None or not cap.isOpened():
        print("⚠️ Camera not found or offline. Falling back to Simulated Demo Stream.")
        cap = MockCamera(CAM_WIDTH, CAM_HEIGHT)
